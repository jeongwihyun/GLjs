(function( window ){
	...
	GL.extend(GL.Slider, {
		render: function( opt ){
			...
			this.sThemeName = this._options.theme == ''?'':'-'+this._options.theme;
			...
			if( this._options.orientation == 'horizontal' ){
				if( 
					...
					this.sliderSize.max = parseInt(con.clientWidth, 10);
				}	
			}else if( this._options.orientation == 'vertical' ){
				
				this._options.height = opt.height? opt.height : '100%';
				this._options.width = opt.width? opt.width : '10px';
				con.style.height = this._options.height;
				
				if( 
					parseInt(con.clientHeight, 10) <= 0 &&
					GL.userAgent() == 'Internet Explorer'
				){
					this.sliderSize.max = parseInt(con.parentElement.clientHeight, 10);
				}else{
					this.sliderSize.max = parseInt(con.clientHeight, 10);
				}
			}
			...
			if( this._options.orientation == 'horizontal' ){
				...
				this._defaultSliderLayout_vertical();
			}
		...
		,_setSliderEvent: function(){
			var _self = this;
			var elem = document.querySelectorAll('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-handle');
			if( elem.length ){
		...
		,_getHTMLContainer: function(){
			...
			var opt = this._options;
			var aHtml = [];
			aHtml.push('<div id="__sliderCenter_'+this.sContainer+'" class="GL'+this.sThemeName+'-slider-range GL'+this.sThemeName+'-slider-corner-all"></div>');
			aHtml.push('<a href="javascript:void(0);" id="__sliderHandle1_'+this.sContainer+'" class="GL'+this.sThemeName+'-slider-handle GL'+this.sThemeName+'-slider-corner-all">');
			if( opt.tooltip ){
				aHtml.push('<span class="GL'+this.sThemeName+'-slider-handle-tip"></span>');
			}
			aHtml.push('</a>');
			if(opt.range.constructor != String && opt.range){
				aHtml.push('<a href="javascript:void(0);" id="__sliderHandle2_'+this.sContainer+'" class="GL'+this.sThemeName+'-slider-handle GL'+this.sThemeName+'-slider-corner-all">');
				if( opt.tooltip ){
					aHtml.push('<span class="GL'+this.sThemeName+'-slider-handle-tip"></span>');
				}
				aHtml.push('</a>');
		...
		,_defaultSliderLayout: function(){
			var con = document.getElementById(this.sContainer);
			var opt = this._options;
			var verticalClass = opt.orientation=='vertical'? ' GL'+this.sThemeName+'-slider-vertical ':' GL'+this.sThemeName+'-slider-horizontal ';
			var gapList = this.sliderSize.gapList;
			con.className = 'GL'+this.sThemeName+'-slider-container GL'+this.sThemeName+'-slider-corner-all '+verticalClass;
			con.style.height = opt.height;
			con.style.width = opt.width;
			
			// set range div
			var elemRange = document.querySelector('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-range');
			...
			if( rangePosition.constructor == String ){
				...
				var oElem = document.querySelector('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-handle');
				oElem.style.left = gapList[idx] + 'px';
			
				if( rangePosition == 'min' ){
					elemRange.className = 'GL'+this.sThemeName+'-slider-range GL'+this.sThemeName+'-slider-corner-all GL'+this.sThemeName+'-slider-range-min';
					var w = parseInt(oElem.style.left, 10);
					elemRange.style.width = w + 'px';
				}else if( rangePosition == 'max' ){
					elemRange.className = 'GL'+this.sThemeName+'-slider-range GL'+this.sThemeName+'-slider-corner-all GL'+this.sThemeName+'-slider-range-max';
				...
				if( opt.values && opt.values.length === 2 ){
					...
					var oElem = document.querySelectorAll('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-handle');
					...
					if( rangePosition === true ){
						...
						elemRange.className = 'GL'+this.sThemeName+'-slider-range GL'+this.sThemeName+'-slider-corner-all GL'+this.sThemeName+'-slider-range';
						elemRange.style.left = l + 'px';
						elemRange.style.width = w + 'px';
					}
				}else if( opt.value && !opt.range ){
					var idx = [];
					idx.push( opt.value - opt.min );
					if( idx[0] < 0 || isNaN(idx[0]) ){ idx[0] = 0; }
					
					this.aSlideRangeVal = [ gapList[idx[0]] ];
					this.aSlideRangeIdx = idx;
					
					var oElem = document.querySelectorAll('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-handle');
					oElem[0].style.left = gapList[parseFloat(opt.value) - opt.min] + 'px';
				}	
			}
		}
		,_defaultSliderLayout_vertical: function(){
			var con = document.getElementById(this.sContainer);
			var opt = this._options;
			var verticalClass = opt.orientation=='vertical'? ' GL'+this.sThemeName+'-slider-vertical ':' GL'+this.sThemeName+'-slider-horizontal ';
			var gapList = this.sliderSize.gapList;
			...
			if( rangePosition.constructor == String ){
				...
				var oElem = document.querySelector('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-handle');
				oElem.style.top = gapList[idx] + 'px';
			
				if( rangePosition == 'min' ){
					elemRange.className = 'GL'+this.sThemeName+'-slider-range GL'+this.sThemeName+'-slider-corner-all GL'+this.sThemeName+'-slider-range-min';
					var h = parseInt(oElem.style.top, 10);
					elemRange.style.height = h + 'px';
				}else if( rangePosition == 'max' ){
					elemRange.className = 'GL'+this.sThemeName+'-slider-range GL'+this.sThemeName+'-slider-corner-all GL'+this.sThemeName+'-slider-range-max';
					var h = this.sliderSize.max - parseInt(oElem.style.top, 10);
					elemRange.style.height = h + 'px';
				}
			}else if( rangePosition.constructor == Boolean ){
				...
				if( opt.values && opt.values.length === 2 ){
					...
					var oElem = document.querySelectorAll('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-handle');
					oElem[0].style.top = gapList[idx[0]] + 'px';
					if(opt.range){
						oElem[1].style.top = gapList[idx[1]] + 'px';
					}
					...
					if( rangePosition === true ){
						...
						var w = gapList[idx[1]] - gapList[idx[0]];
						elemRange.className = 'GL'+this.sThemeName+'-slider-range GL'+this.sThemeName+'-slider-corner-all GL'+this.sThemeName+'-slider-range';
						elemRange.style.top = l + 'px';
						elemRange.style.height = w + 'px';
					}
				}else if( opt.value && !opt.range ){
					var idx = [];
					idx.push( opt.value - opt.min );
					...
					var oElem = document.querySelectorAll('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-handle');
					oElem[0].style.top = gapList[parseFloat(opt.value) - opt.min] + 'px';
				}	
			}
		}
		,_setRangeArea: function( posX, idx, sFirst ){
			...
			if( this._options.orientation == 'horizontal' ){
				...
				if( rangePosition.constructor == String ){
					if( rangePosition == 'min' ){
						elemRange.style.height = posX + 'px';
					}else if( rangePosition == 'max' ){
						var w = this.sliderSize.max - posX;
						elemRange.style.height = w + 'px';
					}
					...
					if( rangePosition === true ){
						if( sFirst ){
							...
							if(posX===0){ this.aSlideRangeVal[0] = posX; }
							if(idx==0 || idx){ this.aSlideRangeIdx[0] = idx; }
							elemRange.style.top = posX + 'px';
							elemRange.style.height = (this.aSlideRangeVal[1] - posX) + 'px';
						}else{
							if(posX){ this.aSlideRangeVal[1] = posX; }
							if(idx==0 || idx){ this.aSlideRangeIdx[1] = idx; }
							elemRange.style.height = (posX - this.aSlideRangeVal[0]) + 'px';
						}
					}
				}
			}
		...
		,_setEvent: function( elem ){
			...
			elem.onmousedown = function(e) { 
				...
				if( _self._options.orientation == 'horizontal' ){
					...
					if( oElem.id == _self.sContainer ){
						_self._controlHandle_vertical(e, this);
						return false;
					}else if( oElem.id == '__sliderCenter_'+_self.sContainer ){
						_self._controlHandle_vertical(e, this);
						return false;
				...
				document.onmousemove = function(e) {
					...
					if( _self._options.orientation == 'horizontal' ){
						...
						var val = parseInt(e.clientY - oElem.mouseY, 10)+gapHalf;
						var moveVal = oElem.startY + val;
						var idx = parseInt(( (_self._options.max - _self._options.min) * moveVal ) / _self.sliderSize.max, 10);
					...
					if( _self.slideIdx != (_self._options.min + idx) ){
						if( _self._options.slide && _self._options.slide.constructor == Function ){
							...
							_self._setTooltipValue(oElem, returnVal, returnValues, oElem.firstHandle);
						}
				...
				document.onmouseup = function(e) {
					...
					if( _self._options.change && _self._options.change.constructor == Function ){
						...
						_self._setTooltipValue(oElem, val, returnValues, oElem.firstHandle);
					}
		...
		,_eventListenerTransitionEnd: function(e){
			if( this._this ){
				GL.removeClass(this, 'GL'+this._this.sThemeName+'-slider-handle-slide');
			}
		...
		,_controlHandle: function( e, target ){
			...
			var elemHandle = document.querySelectorAll('#'+this.sContainer+' .GL'+this.sThemeName+'-slider-handle');
			if( elemHandle.length ){
				...
				GL.addClass(moveElem, 'GL'+this.sThemeName+'-slider-handle-slide');
				...
				if( this.slideIdx != (this._options.min + idx) ){
					if( this._options.slide && this._options.slide.constructor == Function ){
						...
						this._setTooltipValue(moveElem, returnVal, returnValues, firstHandle);
					}
		...
		,_setTooltipValue: function(elem, value, values, handleFirst ){
			if( this._options.tooltip ){
				if( values && handleFirst ){
					elem.children[0].innerHTML = values[0];
				}else if( values && !handleFirst ){
					elem.children[0].innerHTML = values[1];
				}else{
					elem.children[0].innerHTML = value;
				}
			}
		}
		,_controlHandle_vertical: function( e, target ){
			...
			if( elemHandle.length ){
				if(this._options.range == true){
					...
					if( target.id == '__sliderCenter_'+this.sContainer ){
						y = parseInt(elemHandle[0].style.top, 10) + y;
					}
				
					if (isNaN(parseInt(elemHandle[0].style.top))) { elemHandle[0].style.top = '0px'; }
					if (isNaN(parseInt(elemHandle[1].style.top))) { elemHandle[1].style.top = '0px'; }
					leftVal = parseInt(elemHandle[0].style.top);
					rightVal = parseInt(elemHandle[1].style.top);
					if( y <= leftVal ){
						moveElem = elemHandle[0];
						firstHandle = true;
					}else{
						if( y > leftVal && ((rightVal - leftVal)/2) > (y - leftVal) ){
							moveElem = elemHandle[0];
							firstHandle = true;
						}else if( y > leftVal && ((rightVal - leftVal)/2) < (y - leftVal) ){
							moveElem = elemHandle[1];
							firstHandle = false;
						}else{
							moveElem = elemHandle[1];
				...
				var gapHalf = (this.sliderSize.gap / 2);
				var gapList = this.sliderSize.gapList;
				var val = parseInt(y - moveElem.mouseY, 10)+gapHalf;
				var moveVal = moveElem.startY + val;
				var idx = parseInt(( (this._options.max - this._options.min) * moveVal ) / this.sliderSize.max, 10);
				
				GL.addClass(moveElem, 'GL'+this.sThemeName+'-slider-handle-slide');
				moveElem.style.top = gapList[idx]+'px';
				moveElem.focus();
